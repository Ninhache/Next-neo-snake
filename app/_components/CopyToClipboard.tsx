...
import getRefCurrentPtr from "@/lib/getRefCurrentPtr";
import { cn } from "@/lib/utils";
import { PropsWithChildren, useRef, useState } from "react";

const CopyToClipboard = ({ children }: PropsWithChildren) => {
  ...
  const onCopy = () => {
    ...
    setCopied(true);
    if (textInputInstance && textInputInstance.textContent !== null)
      navigator.clipboard.writeText(textInputInstance.textContent);
    ...
    >
      {
        <button
          className={cn(
            "absolute right-2 top-2 h-8 w-8 rounded border-2 bg-gray-700 p-1 transition-[opacity] dark:bg-gray-800",
            {
